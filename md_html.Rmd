...
- Adding two new variables:
  - `length_description` and `age` (Time since original air date)
    - At time of starting this (Late September), Season 34 already airing so if any episode `age` analysis is conducted will be based off this date as newer seasons [start around this time](https://en.wikipedia.org/wiki/List_of_The_Simpsons_episodes) 
...
Household Viewer - 8.6m household
c
```

- Simply refactor by `as.numeric()` for further application
- Missing data points, originally missed bc of type issue
  - `is.na()` didn't detect b/c it was always char type
  - Data is missing from wikipedia
    - Interesting observation made -- Wikipedia articles report viewership in both (presumed) individual viewers and viewing households
      - i.e. Compare the table of episode / viewer to individual episode rating under reception for Nielsen rating and viewing houeholds
      - Ratio comparing existing data of viewers to viewing household came to be about 1.7-8:1
      - For case of season 8, 
  - S8 E7,8,20
