...
# Remove titles
long_writer$writers <- str_remove_all(long_writer$writers,'(Story|Teleplay)\\sby(\\s)\\:\\s')

# Remove special chars
writers_clean <- base::strsplit(long_writer$writers, split='\\s&\\s|,|(\\s&)')
View(writers_clean)
View(data.frame(unlist(writers_clean)))


# Reformat
writers_unlist <- unlist(writers_clean)
writers_unlist <- str_trim(writers_unlist, "left")

View(data.frame(writers_unlist))

# Extracted
temp_regex <- 
'([A-Z][a-z]+\\s[A-Z][a-z]+)|
([A-Z][a-z]+\\s[A-Z]\\.\\s[A-Z][a-z]+)|
([A-Z][a-z]+\\s[A-Z][a-z]+[:punct:][A-Z][a-z]+)
([A-Z][:punct:]\\s[A-Z][a-z]+\\s[A-Z][a-z]+'


writers_ex <- unlist(str_extract_all(writers_unlist, temp_regex))

View(data.frame(writers_ex))
...
- Otherwise 123 instance of episodes of multiple writers
  ...
  - 3rd Pass:
- Issue with RegEx where one writer credited with middle name rather than two
